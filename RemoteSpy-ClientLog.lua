...
local LazyFix = loadstring(game:HttpGet("https://raw.githubusercontent.com/qian-cheng-awa/Tools/refs/heads/main/TableToLua.lua"))() -- Very lazy fix as I'm legit just pasting it from the rewrite
...
function genScript(remote, args, client)
	...
	if #args > 0 then
		xpcall(function()
			Args = string.sub(LazyFix.Convert(args, true),2,-2)
		end,function(err)
			xpcall(function()
				for i, v in next, args do
					...
					elseif type(v) == "userdata" and typeof(v) ~= "Instance" then
						Args = Args .. string.format("nil --[[%s]]", typeof(v))
					elseif type(v) == "userdata" then
						Args = Args .. "game." .. v:GetFullName()
					end
				end
				Args ..= "\n"
			end,function()
...
function remoteHandler(data)
	...
	if (data.remote:IsA("RemoteEvent") or data.remote:IsA("UnreliableRemoteEvent")) then
		...
		elseif lower(data.method) == "onclientevent" then
			newRemote("clientevent", data)
		end
	elseif data.remote:IsA("RemoteFunction") then
		if lower(data.method) == "invokeserver"  then
			newRemote("function", data)
		elseif lower(data.method) == "onclientinvoke" then
			newRemote("clientfunction", data)
		end
	end
end
...
local function disablehooks()
	if synv3 then
		...
		unhook(getrawmetatable(game).__index,originalindex)
		...
		if hookmetamethod then
			...
			hookfunction(getrawmetatable(game).__index,originalindex)
		end
...
function toggleSpy()
	if not toggle then
		...
		if synv3 then
			...
			oldindexcall = hook(getrawmetatable(game).__newindex,clonefunction(newindexcall))
			oldindex = hook(getrawmetatable(game).__index,clonefunction(indexcall))
		else
			if hookmetamethod then
...
newButton(
	"Logcheckcaller",
	function() return ("[%s] Log remotes fired by the client"):format(configs.logcheckcaller and "ENABLED" or "DISABLED") end,
	function()
...
newButton(
	"Advanced Info",
	function() return ("[%s] Display more remoteinfo"):format(configs.advancedinfo and "ENABLED" or "DISABLED") end,
	function()
